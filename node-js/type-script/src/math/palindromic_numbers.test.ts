...
describe('generatePalindromicNumbers2', () => {
  it('should generate palindromic numbers in base 10', () => {
    const generator = generatePalindromicNumbers2();
    expect(generator.next().value).toBe(0);
    expect(generator.next().value).toBe(1);
    expect(generator.next().value).toBe(2);
    expect(generator.next().value).toBe(3);
    ...
    const generator1 = generatePalindromicNumbers();
    const generator2 = generatePalindromicNumbers2();

    for (let i = 0; i < 1000; i++) {
      expect(generator2.next().value).toBe(generator1.next().value);
    }
  });

  it('should generate palindromic numbers in base 2', () => {
    const generator = generatePalindromicNumbers2(2);
    expect(generator.next().value).toBe(0); // 0
    expect(generator.next().value).toBe(1); // 1
    expect(generator.next().value).toBe(3); // 11
    expect(generator.next().value).toBe(5); // 101
  ...
  it('should throw an error for invalid base', () => {
    expect(() => generatePalindromicNumbers2(1).next()).toThrow('Base must be an integer greater than or equal to 2.');
    expect(() => generatePalindromicNumbers2(1.5).next()).toThrow(
